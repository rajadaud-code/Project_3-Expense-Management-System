...
//     return res
//       .status(400)
//       .json({ message: "Amount, type, and transactionDate are required" });
//   }
...
export const createTransaction = async (req, res) => {
  ...
  try {
    // ⭐ CHANGE START
    // Prevent expenses from exceeding available income
    if (type === "expense") {
      ...
      const totalIncome = Number(balanceResult.rows[0].total_income);
      const totalExpense = Number(balanceResult.rows[0].total_expense);
      const availableBalance = totalIncome - totalExpense;

      if (Number(amount) > availableBalance) {
        return res.status(400).json({
          message: `Insufficient balance. Available balance is ${availableBalance}.`,
        });
      }
    }
    // ⭐ CHANGE END
...
export const updateTransaction = async (req, res) => {
  ...
  try {
    // ⭐ CHANGE START: Get existing transaction
    ...
    const newAmount =
      amount !== undefined ? Number(amount) : Number(oldTransaction.amount);

    const newType = type || oldTransaction.type;

    // ⭐ CHANGE: Validate only when transaction is an expense
    if (newType === "expense") {
      const balanceResult = await pool.query(
        `
        SELECT
      ...
      const totalIncome = Number(balanceResult.rows[0].total_income);
      let totalExpense = Number(balanceResult.rows[0].total_expense);

      // ⭐ CHANGE: Remove old expense before checking
      if (oldTransaction.type === "expense") {
        totalExpense -= Number(oldTransaction.amount);
      }
