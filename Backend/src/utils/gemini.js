...
Return ONLY valid JSON (no markup):
{
...
Provide exactly 4 tips. Each tip should reference an actual category from the data and include a realistic monetary value.`;
...
export const analyzeTransactionList = async ({
  ...
  const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight. Focus on patterns, notable categories, and actionable advice. Use actual numbers from the data.
...
export const analyzeBudgetList = async ({ budgets, currency = "USD" }) => {
  const lines = budgets.map((b) => {
    ...
    return `Budget ID ${b.id} | Category: ${b.category_name} | Limit: ${currency} ${total.toFixed(2)} | Spent: ${currency} ${spent.toFixed(2)} | Usage: ${pct}%`;
  });

  const prompt = `You're a personal finance assistant. Analyze each budget below and provide a one-sentence summary.

Budgets:
${lines.join("\n")}

For each budget, return:
- status: 'good' (well-paced, under 70%), 'caution' (approaching limit or above 70%), or 'concerning' (over 100%)
